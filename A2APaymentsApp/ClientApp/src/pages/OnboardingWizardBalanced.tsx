...
function OnboardingWizard() {
  ...
  // Get compliance variant from context, default to modal
  const complianceVariant = entryContext.complianceVariant || 'modal'
  ...
  // Navigation handlers
  const handleNext = () => {
    // If transitioning to ReviewConsent and modal variant, show compliance modal first
    if (currentScreen === OnboardingScreen.SelectAccount && 
        currentScreen + 1 === OnboardingScreen.ReviewConsent &&
        complianceVariant === 'modal' && 
  ...
  return (
    <div className="onboarding-wizard-v2">
      {/* Compliance Disclosure - Modal variant */}
      {complianceVariant === 'modal' && (
      ...
      {/* Compliance Disclosure - Fullscreen variant */}
      {complianceVariant === 'fullscreen' && !complianceAcknowledged && (
        <ComplianceDisclosure
          variant="fullscreen"
          isOpen={true}
          onAcknowledge={() => setComplianceAcknowledged(true)}
        />
      )}
      
      {/* Only show wizard if fullscreen compliance is acknowledged or not fullscreen */}
      {(complianceVariant !== 'fullscreen' || complianceAcknowledged) && (
        <>
          {showStepper && renderStepper()}
          <div className="onboarding-main-content">
            {/* Compliance Disclosure - Banner variant */}
            {complianceVariant === 'banner' && (
              <ComplianceDisclosure
                variant="banner"
                isOpen={true}
                onAcknowledge={() => {}}
