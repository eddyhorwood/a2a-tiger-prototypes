# Pact testing for Payment Execution Service

This project contains the pact consumer tests for the Payment Execution Service.
...
These tests provide only the Pact configuration for the PaymentExecutionService consumer. See for example  the [PaymentExecutionServiceFailConsumerTests](../tests/PaymentExecutionService.ConsumerPactTests/PayExeServiceFailPaymentRequestConsumerTests.cs).
...
```shell
docker run -v ./tests/PaymentExecutionService.ConsumerPactTests/pacts:/pacts pactfoundation/pact-cli pact-broker publish /pacts --consumer-app-version 0.0.1 --branch local-test --broker-base-url https://xero.pactflow.io --broker-token  <your-token-from-pact-flow>
```
Command to list the pact versions
```shell
docker run -v ./tests/PaymentExecutionService.ConsumerPactTests/pacts:/pacts pactfoundation/pact-cli pact-broker list_latest_pact_versions  --broker-base-url https://xero.pactflow.io --broker-token  <your-token-from-pact-flow>
```
Command to get pact version for a specific consumer 
```shell
docker run -v ./tests/PaymentExecutionService.ConsumerPactTests/pacts:/pacts pactfoundation/pact-cli pact-broker describe_version  --broker-base-url https://xero.pactflow.io --broker-token  <your-token-from-pact-flow> --pacticipant=PaymentExecutionService 
```
## Provider states
